...
public interface EventRepository extends JpaRepository<EventEntity, Long> {
    @Query("SELECT e FROM Event e WHERE e.user.id = :userId AND e.user.amount > 0 ORDER BY e.date DESC")
    List<EventEntity> findMyEvents(@Param("userId") Long userId);

    @Query("SELECT e FROM EventEntity e WHERE e.user.id = :userId AND e.user.amount > 0 AND e.date = :date And e.eventType = :type ORDER BY e.date DESC")
    List<EventEntity> findMyEventSummaries(@Param("userId") Long userId, @Param("type") String type, @Param("date") String date);

    @Query("SELECT e FROM EventEntity e WHERE e.user.id = :userId AND e.user.amount < 0 ORDER BY e.date DESC")
